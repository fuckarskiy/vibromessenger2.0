...
def register_step1():
    d=request.json; return jsonify({"exists": bool(db().cursor().execute("SELECT id FROM users WHERE username=?",(d["username"],)).fetchone())})
...
def register_step2():
    ...
    except sqlite3.IntegrityError: return {"error":"exists"},400

@app.route("/login_step1", methods=["POST"])
def login_step1():
    d=request.json; c=db()
    row=c.cursor().execute("SELECT id FROM users WHERE username=?",(d["username"],)).fetchone(); c.close()
    return {"exists":bool(row)}

@app.route("/login_step2", methods=["POST"])
def login_step2():
    d=request.json; c=db()
    row=c.cursor().execute("SELECT id, username, avatar FROM users WHERE username=? AND password_hash=?",(d["username"],hash_pw(d["password"]))).fetchone()
    c.close()
